...
public class ComentarioServiceImpl implements ComentarioService {
	...
	public HttpStatus createComment(ComentarioDTO comentarioDTO) {
		Optional<Utilizador> userDB = this.utilizadorRepository.findById(comentarioDTO.getUserId());
		if (!userDB.isPresent()) {
		...
		Comentario comentario = comentBuilder.setDescricao(comentarioDTO.getDescricao())
	...
	public HttpStatus updateComment(long id, ComentarioDTO comentarioDTO) {
		...
		if (CommentDB.isPresent()) {
			...
			if (comentarioDTO.getDescricao() != null)
				commentUpdate.setDescricao(comentarioDTO.getDescricao());
			if (comentarioDTO.getSandwishId() != 0)
	...
	public List<ComentarioDTO> getAllComments() {
		...
		return ObjectMapperUtils.mapAll(listaComentarios, ComentarioDTO.class);
	}
	...
	public ComentarioDTO getCommentById(long commentId) {
		...
		if (commentDB.isPresent()) {
			return ObjectMapperUtils.map(commentDB.get(), ComentarioDTO.class);
		} else {
